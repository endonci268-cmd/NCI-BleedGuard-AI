...
def get_data():
    try:
        ...
        # ttl=0 เพื่อให้อ่านข้อมูลสดใหม่ ไม่ค้าง cache
...
# --- 5. ส่วนบันทึกข้อมูล (ใช้ clear_on_submit=False เพื่อป้องกันข้อมูลส่งไม่ครบ) ---
...
# --- 6. ส่วนประมวลผล (AI + Clinical Override) และบันทึกข้อมูล ---
if submit_button:
    if ai_model and case_id != "ENDO-NCI-":
        try:
            ...
            # --- CLINICAL OVERRIDE (พยาบาลสั่งการทับ AI เพื่อความปลอดภัย 100%) ---
            # บังคับแดงถ้าเป็น EMR หรือกินยาละลายลิ่มเลือดพร้อมติดคลิป
            ...
            # บังคับเหลืองถ้ามีการติดคลิปหรือแผลใหญ่ (แม้ AI จะให้เขียว)
            elif hemoclip == "ใส่" or size >= 2.0:
                risk, color, text_color, action = "YELLOW", "#FFCC00", "black", "⚠️ โทรติดตามที่ 24, 48 ชม. (เฝ้าระวังคลิป/ขนาดแผล)"
            # กรณีอื่นๆ ใช้ตามคะแนน AI
            elif prob >= 0.40: 
            ...
            st.success(f"✅ บันทึกข้อมูลเคส {case_id} ลงระบบเรียบร้อยแล้ว!")
            
            # ปุ่มเพื่อล้างค่าหน้าจอและอัปเดต Dashboard ด้านล่างทันที
...
if not df_history.empty:
    ...
    with g1:
        ...
        fig_trend = px.bar(daily_trend, x='Date_Only', y='Count', color='Risk_Level',
                           ...
                           barmode='stack', height=350, title="แนวโน้มความเสี่ยงรายวัน")
        st.plotly_chart(fig_trend, use_container_width=True)
    with g2:
        st.write("📋 รายละเอียดหัตถการ (เรียงจากล่าสุด)")
