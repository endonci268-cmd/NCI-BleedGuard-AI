...
# --- 5. ส่วนบันทึกข้อมูล (ใช้ Form เดียว ป้องกัน Error) ---
...
# --- 6. ส่วนประมวลผล (AI + Clinical Override แบบสอดคล้อง) ---
if submit_button:
    if ai_model and case_id != "ENDO-NCI-":
        try:
            ...
            # ปรับคะแนนให้สอดคล้องกับความเสี่ยงหน้างานจริง
            
            # เงื่อนไข RED: EMR หรือ ยา+คลิป หรือ ประวัติฉายแสง
            if procedure == "EMR" or (medication == "ใช่" and hemoclip == "ใส่") or radiation == "ใช่":
                risk, color, text_color, action = "RED", "#FF4B4B", "white", "🚨 โทรติดตามที่ 24, 48, 72 ชม. (ปัจจัยเสี่ยงสูงเฉพาะราย)"
                if score_percent < 85: score_percent = 85.0 # ดันเข็มเข้าโซนแดง
            
            # เงื่อนไข YELLOW: ติดคลิป (แม้ไม่มีปัจจัยอื่น) หรือ แผลใหญ่
            elif hemoclip == "ใส่" or size >= 2.0:
                risk, color, text_color, action = "YELLOW", "#FFCC00", "black", "⚠️ โทรติดตามที่ 24, 48 ชม. (เฝ้าระวังตำแหน่งคลิป/ขนาดแผล)"
                if score_percent < 35: score_percent = 35.0 # ดันเข็มเข้าโซนเหลือง
            
            # กรณีทั่วไปตามคะแนน AI
            elif prob >= 0.40: 
            ...
            # 3. บันทึกข้อมูล (ส่งค่าที่ Override แล้วลง Sheets)
            new_entry = pd.DataFrame([{
                ...
                "Medication": medication, "Clip": hemoclip, "Risk_Level": risk, 
                "Score": f"{score_percent:.2f}%", "Advice": action
            }])
            
            conn = st.connection("gsheets", type=GSheetsConnection)
            conn.update(worksheet="Sheet1", data=pd.concat([df_history, new_entry], ignore_index=True))
            st.toast(f"✅ บันทึกข้อมูล {case_id} ครบถ้วน")
            
            # ปุ่มล้างหน้าจอและ Refresh Dashboard
            if st.button("🔄 บันทึกสำเร็จ! คลิกอัปเดต Dashboard"):
                st.rerun()
...
if not df_history.empty:
    ...
    c2.metric("🟢 GREEN", len(df_filtered[df_filtered['Risk_Level'] == 'GREEN']))
    c3.metric("🟡 YELLOW", len(df_filtered[df_filtered['Risk_Level'] == 'YELLOW']))
    c4.metric("🔴 RED", len(df_filtered[df_filtered['Risk_Level'] == 'RED']))

    st.write("📋 รายละเอียด 10 เคสล่าสุด (เรียงจากล่าสุด)")
    styled_df = df_filtered.sort_values('Timestamp', ascending=False).head(10)[['Timestamp', 'Case_ID', 'Risk_Level', 'Score', 'Advice']]
    st.dataframe(
