...
def get_data():
    try:
        conn = st.connection("gsheets", type=GSheetsConnection)
        # ตั้งค่า ttl=0 เพื่อให้อ่านข้อมูลสดใหม่จาก Sheets เสมอ
...
# --- 5. ส่วนบันทึกข้อมูล (ปิด clear_on_submit เพื่อประมวลผลก่อน) ---
with st.form("triage_form", clear_on_submit=False):
    st.markdown("#### 📝 บันทึกข้อมูลแยกตามหมวดหมู่")
    tab1, tab2, tab3 = st.tabs(["👤 ข้อมูลทั่วไป & ประวัติ", "🔍 รายละเอียดหัตถการ", "💊 ปัจจัยเสี่ยง & Clip"])
...
# --- 6. ส่วนแสดงผลลัพธ์และบันทึกข้อมูล ---
if submit_button:
    if ai_model and case_id != "ENDO-NCI-":
        try:
            ...
            # กำหนดเกณฑ์ (Clinical Override สามารถเพิ่มเงื่อนไขตรงนี้ได้)
            if prob >= 0.40: risk, color, text_color, action = "RED", "#FF4B4B", "white", "🚨 โทรติดตามที่ 24, 48, 72 ชม."
            elif prob >= 0.11: risk, color, text_color, action = "YELLOW", "#FFCC00", "black", "⚠️ โทรติดตามที่ 24, 48 ชม."
            else: risk, color, text_color, action = "GREEN", "#28A745", "white", "✅ ให้คู่มือสังเกตอาการตามมาตรฐาน"

            # แสดง Gauge และ Alert
            res_col1, res_col2 = st.columns([1.2, 1])
            with res_col1:
                fig_gauge = go.Figure(go.Indicator(
                    mode = "gauge+number", value = score_percent,
            ...
            conn = st.connection("gsheets", type=GSheetsConnection)
            conn.update(worksheet="Sheet1", data=pd.concat([df_history, new_entry], ignore_index=True))
            st.toast(f"✅ บันทึกข้อมูล {case_id} เรียบร้อยแล้ว")
            
            # ปุ่มเพื่อล้างค่าและอัปเดต Dashboard
...
if not df_history.empty:
    ...
    with g2:
        ...
        styled_df = df_filtered.sort_values('Timestamp', ascending=False)[['Timestamp', 'Case_ID', 'Risk_Level', 'Score']]
        st.dataframe(
            styled_df.style.map(highlight_risk, subset=['Risk_Level']), 
            use_container_width=True, 
            hide_index=True
    ...
    st.info("💡 ยังไม่มีข้อมูลในระบบ เริ่มบันทึกเคสแรกได้เลยครับ") 
